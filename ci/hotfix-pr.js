...
const createPullRequest = async (title) => {
	try {
		...
		await runCommand(`gh pr create --base master --title "${title}" --body "Hotfix done, please review the PR and merge it with master for production deployment."`);
...
const main = async () => {
	const prTitle = process.argv[2] || "Hotfix Done";
